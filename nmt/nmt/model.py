...
class BaseModel(object):
    ...
    @abc.abstractmethod
    def _build_encoder(self, hparams, src_vocab_table):
...
class Model(BaseModel):
    ...
    def _build_encoder(self, hparams):
        ...
        #
        # source = tf.Print(iterator.source, [iterator.source], summarize=100)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            with open('/data/data1/users/amandalios/myrto-tsokanaridou-chatbot/topic_modeling/ids_to_topic_vectors.pickle', 'rb') as handle:
                mapper = pickle.load(handle)

            vf = np.vectorize(lambda id, pos: np.float32(mapper[id][pos]))

            topic_vectors = tf.py_func(lambda x: np.concatenate((vf(x, 0), vf(x, 1), vf(x, 2)), axis=2), [expanded_source], dtype)
            encoder_emb_inp = tf.concat([topic_vectors, encoder_emb_inp], axis=2)
            encoder_emb_inp.set_shape((None, None, 515))
            # encoder_emb_inp = tf.Print(encoder_emb_inp, [encoder_emb_inp], message='Encoder embedding input value: \n', summarize=1000)
