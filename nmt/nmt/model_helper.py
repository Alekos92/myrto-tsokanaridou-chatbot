...
def _create_or_load_embed(embed_name, vocab_file, embed_file,
    ...
    with open(
            ...
            'rb') as handle:
        mapper = np.array(pickle.load(handle))
        print('MAPPER SHAPE IS : {}'.format(mapper.shape))
    ...
    if vocab_file and embed_file:
        ...
        with tf.device(_get_embed_device(vocab_size)):
            prefix = tf.constant(mapper, dtype=tf.float32)
            print('PREFIX SHAPE: {}'.format(prefix.shape))
            embedding = tf.get_variable(
                embed_name, [vocab_size, embed_size - 3], dtype)
            modified_embedding = tf.concat([prefix, embedding], axis=1)
            print('MODIFIED EMBEDDING SHAPE IS : {}'.format(modified_embedding.shape))
